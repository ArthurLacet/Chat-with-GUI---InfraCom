...
ABS_FILE_PATH_FILES = os.path.join(SCRIPT_DIR, r"inbox")
...
class receiver:
    ...
    def setupSockets(self):
        ...
        self.socket.bind(CLIENT_A_RECEIVER_ADDR)
        print(f"Client A receiver created at {CLIENT_A_RECEIVER_ADDR}")

        self.socket.listen(1)
        self.con, self.addr = self.socket.accept()
        print(f"Client A receiver connected with {self.addr}")

        
    def mainLoop(self):
        print("MainLoop is:     Ready")
        while True:
            try:
                msg = self.con.recv(1024)
                msg = msg.decode('utf-8')
                print(f"Client A received:   {msg}")
                if msg == 'ªend':
                    self.con.close()
