...
MYPFP = r'GUI_MePfp.png'
MYNICK = r'Eu'
MYSTATUS = r'Online'
MYCUSTOMSTATUS = r'Fazendo nada'

FRIENDPFP = r'Gui_CauePfp.png'
FRIENDNICK = r'Cauê'
FRIENDSTATUS = r'Away'
FRIENDCUSTOMSTATUS = r'Férias, pelo amor de Deus!'

# Lista de extensões, não é o melhor jeito de fazer, mas serve por equanto
IMAGEEXT = [
            '.jpg', '.jpeg', '.jpe', '.jif', '.jfif', '.jfi', '.png',
            '.webp', '.tiff', '.tif', '.psd', '.raw', '.arw', '.cr2', 
            '.nrw', '.k25','.svg', '.svgz', '.jp2', '.j2k', '.jpf', 
            '.jpx', '.jpm', '.mj2', '.bmp', '.dib','.heif', '.heic', 
            '.ind', '.indd', '.indt', '.jp2', '.j2k', '.jpf', '.jpx', 
            '.jpm', '.mj2'
            ]

AUDIOEXT = [
            '.wv', '.wma', '.webm', '.wav', '.vox', '.voc', '.tta', 
            'sln', 'rf64', '.raw', '.ra', '.rm', '.opus', '.ogg', 
            '.oga', '.mogg', '.nmf', '.msv', '.mpc', '.mp3', '.mmf',
            '.m4p', '.m4b', '.m4a', '.ivs', '.iklax', '.gsm', '.flac', 
            '.dvf', '.dss', '.cda', '.awb', '.au', '.ape', '.alac', 
            '.aiff', '.act', '.aax', '.aac', '.aa', '.8svx', '.3gp'
            ]
# Icon do cursor
CURSOR = 'hand2'
...
class Gui():
    def __init__(self):
        ...
        _title = "Discord powered by Gmail"
        ...
        self.r.protocol("WM_DELETE_WINDOW", self.onclosing)
    ...
    def _setupWidgetsTopFrame(self):
        ...
        self.pausebuttonimg = ImageTk.PhotoImage(Image.open(r'Gui_ResumeIcon.png'))
    ...
    def _setupWidgetsMiddleFrame(self):
        ...
        # Algumas variaveis, n quero inicia-las no init
        self.lastcurtime = ''
        self.msgBuffer = []
        self.vlcFrames = []
        self.vlcInstances = []
    ...
    def _setupWidgetsBottomFrame(self):
        ...
        self.imgbtup = ImageTk.PhotoImage(Image.open(r"Gui_UploadIcon.png"))
        self.imgbtclear = ImageTk.PhotoImage(Image.open(r"Gui_ClearIcon.png"))
        ...
        self.imgbuttonup = ImageTk.PhotoImage(Image.open(r"Gui_PlusIcon.png"))
        ...
        self.imgbuttonsend = ImageTk.PhotoImage(Image.open(r"Gui_SendIcon.png"))
    ...
    def upload(self):
        try:
            ...
            for file in self.files:
                ...
                if file[-4:] in IMAGEEXT or file[-5:] == '.jpeg':
    ...
    def printfunction(self, me=True, time='', msg='', msgtype='text'):
        ...
        if msgtype == 'text':
            ...
            self.mediaFrames.append(tkinter.Frame(
                master=self.text_MiddleFrame, height=mediaframe_h, width=mediaframe_w, bg=LBLUE, highlightthickness=0, border=0))
            self.text_MiddleFrame.window_create(
                END, window=self.mediaFrames[-1])

            self.vlcFrames.append(tkinter.Frame(
                master=self.mediaFrames[-1], height=vlcframe_h, width=vlcframe_w, bg='black'))
            ...
            self.vlcButtons[-1].place(y=vlcbutton_y, width=vlcframe_w, x=2, height=38)
    ...
    #
    # Criei, mas n utilizei, to com pena de apagar :(
    #
    ...
    def setCustomStatus(self, customStatus="Sono"):
